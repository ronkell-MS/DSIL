...
class DecRockSampleState(pomdp_py.State):
    ...
    def __eq__(self, other):
        if isinstance(other, DecRockSampleState):
    ...
    def __repr__(self):
        return "DecRockSampleState(%s|%s|%s)" % (str(self.position), str(self.rocktypes), str(self.terminal))


class DecRockSampleAction(pomdp_py.Action):
    def __init__(self, name):
        self.name = name
    def __hash__(self):
        return hash(self.name)
    def __eq__(self, other):
        if isinstance(other, DecRockSampleAction):
    ...
    def __str__(self):
        return self.name
    def __repr__(self):
        return "DecRockSampleAction(%s)" % self.name

class DecRockSampleObservation(pomdp_py.Observation):
    def __init__(self, quality):
        self.quality = quality
    def __hash__(self):
        return hash(self.quality)
    def __eq__(self, other):
        if isinstance(other, DecRockSampleObservation):
    ...
    def __repr__(self):
        return "DecRockSampleObservation(%s)" % str(self.quality)
...
class DecRockSampleTransitionModel(pomdp_py.TransitionModel):
    ...
    def sample(self, state, action):
        ...
        return DecRockSampleState(next_position, next_rocktypes, next_terminal)
            ...
            for y1 in range(0,self._n):
                for x2 in range(self.specificBound,self._n):
                    for y2 in range(0,self._n):
                        for rock in rocks:
                            flag = False
                            if x1 == self._n-1 and y1 == 0:
                                flag = True
                            if x2 == self._n-1 and y2 == self._n-1:
                                flag = True
...
class DecRockSampleObservationModel(pomdp_py.ObservationModel):
    ...
    def sample(self, next_state, action):
        action_name = action.name
        action_arr = action_name.split('X')
        if not next_state.terminal and checkIfCheckAction(action_name):
            # compute efficiency
            curr_position = (next_state.position[0], next_state.position[1])
            action_index = 0
            if action_arr[0] == 'idle':
            ...
            if not keep:
                ...
                return DecRockSampleObservation(observed_rocktype)
            else:
                return DecRockSampleObservation(actual_rocktype)
        else:
            # Terminated or not a check action. So no observation.
            return DecRockSampleObservation(None)
    ...
    def get_all_observations(self):
        ...
        (e.g. value iteration)"""
        return [DecRockSampleObservation(s)
                for s in {"G", "B"}]



class DecRockSampleRewardModel(pomdp_py.RewardModel):
    ...
    def sample(self, state, action, next_state, normalized=False, **kwargs):
        ...
        if checkIfSampleAction(action_name):
            ...
            rock_index = int(action_arr[action_index][-1]) -1
            rock_pos = self._rock_locs[rock_index]
            if rock_pos[0] == curr_position[0] and rock_pos[1] == curr_position[1] and state.rocktypes[rock_index] == 'G':
                return 10
            else:
                return -10
...
class PolicyModel(pomdp_py.RolloutPolicy):
    ...
    """ACTIONS = {DecRockSampleAction(s)
              for s in {"upXidle","downXidle","leftXidle","rightXidle",'check1Xidle','check2Xidle','check3Xidle','sample1Xidle','sample2Xidle','sample3Xidle' ,
                        'idleXup','idleXdown','idleXleft','idleXright','idleXcheck1','idleXcheck2','idleXcheck3','idleXsample1','idleXsample2','idleXsample3'}}"""
    ACTIONS = {DecRockSampleAction(s)
...
class DecRockSampleProblem(pomdp_py.POMDP):
    ...
    def __init__(self, n, k, init_state, rock_locs, init_belief):
        ...
        agent = pomdp_py.Agent(init_belief,
                               PolicyModel(),
                               DecRockSampleTransitionModel(n, rock_locs, self.in_exit_area),
                               DecRockSampleObservationModel(rock_locs),
                               DecRockSampleRewardModel(rock_locs, self.in_exit_area))
        env = pomdp_py.Environment(init_state,
                                   DecRockSampleTransitionModel(n, rock_locs, self.in_exit_area),
                                   DecRockSampleRewardModel(rock_locs, self.in_exit_area))
        self._rock_locs = rock_locs
        super().__init__(agent, env, name="DecRockSampleProblem")